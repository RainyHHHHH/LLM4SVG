...
from llm4svg.svglib import replace_gradient_tags, remove_svg_tag, apply_g_attributes_to_children
...
def get_svg_dataset(
    ...
    if path_exists(cfg.preprocessed_disk):
        ...
        assert set(cfg.text_prompt).issubset(set(cfg.target_columns)), \
        ...
        logger.info(f"Load dataset {dataset_name}, column names: {dataset.column_names}")
    ...
    if cfg.get('for_recon', False):
        ...
        train_dataset, eval_dataset = processed_dataset, Dataset.from_dict(processed_dataset[:eval_size])
    else:
