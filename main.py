...
from llm4svg.utils import get_runtime_dir, set_cfg_struct, initialize_accelerator, LogUtilsMixinAccelerator
...
def main(cfg: omegaconf.DictConfig):
    ...
    # Seed
    set_seed(cfg.seed, device_specific=cfg.device_specific, deterministic=cfg.deterministic)
    ...
    train_dataset, eval_dataset = get_svg_dataset(cfg.data, cfg.x.data_preprocess, logger=actual_logger)
    accelerator.wait_for_everyone()

    # Methods mapping
    method_map = {
        "LLM4SVG-GPT2": lambda: llm4svg_gpt2_sft(cfg, project_dir, accelerator, train_dataset, actual_logger),
        "LLM4SVG-Phi2": lambda: llm4svg_phi2_sft_via_trl(cfg, project_dir, accelerator, train_dataset, eval_dataset,
                                                         actual_logger),
